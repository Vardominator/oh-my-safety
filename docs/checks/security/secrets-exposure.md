...
**Category:** security · **Default severity:** warn · **Platforms:** macos, linux · **Runs every:** 3600s (in the background agent)
...
- **Leaked `.env` files.** Project `.env` files routinely hold database passwords and API keys. A group/other-readable `.env` exposes them to other local users and processes.
...
5. **Filename scan on macOS** — uses Spotlight (`mdfind`) to search `~/Desktop`, `~/Documents`, `~/Downloads`, and iCloud Drive for names containing `password`, `seed phrase`, `mnemonic`, `recovery phrase`, `private key`, or `2fa` (case- and accent-insensitive). Only plaintext-ish files are flagged — no extension, or `.txt`, `.md`, `.rtf`, `.csv`, `.doc`, `.docx`, `.pages`, `.numbers`, `.xlsx`. Linux runs the permission, credential, history, `.env`, and authorized-key checks but skips this macOS-only pass.
...
The one exception is the macOS Spotlight filename scan. It self-degrades: if `mdfind` isn't available, or a folder (notably iCloud Drive) can't be read without Full Disk Access, that folder is skipped, a single "Filename scan was limited" info line is printed, and the rest of the check runs normally. Granting the macOS background agent Full Disk Access makes the filename scan more thorough but is not required. Linux skips the Spotlight pass.
...
- **"Readable by others" on a single-user machine.** If you're the only account, a group/other-readable file is lower risk than the wording suggests — but it is still surfaced. `ignore` it only after reviewing the risk.
